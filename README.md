...
| --- | --- | --- |
| `uptime_kuma` | An Uptime Kuma status-page slug | `{"up": N, "down": M, "total": T}` |
| `webdav` | A single file's WebDAV last-modified time | `{"result": "OK"\|"STALE", "recent": bool, "timestamp": ..., "age_hours": ..., "threshold_hours": ...}` |
| `docker_updater` | [docker-updater](https://github.com/liquidguru/docker-updater) `/api/status` | `{"pending_updates": N}` |
| `docker_containers` | The Docker socket | `{"running": N, "stopped": M, "total": T}` |
...
```json
{
  "status": "ok",
  "checks": {
    "uptime_kuma": {"status": "ok", "up": 12, "down": 1, "total": 13},
    "webdav": {"status": "ok", "result": "OK", "recent": true, "timestamp": "2026-09-14T08:12:00-04:00", "age_hours": 1.2, "threshold_hours": 24.0},
    "docker_updater": {"status": "ok", "pending_updates": 3},
    "docker_containers": {"status": "ok", "running": 21, "stopped": 2, "total": 23}
  }
}
```
